...
#include "GalacticArmada/Public/AI/ShipAIController.h"
	...
	for (int32 i = 0; i < ThrusterParticleEffects.Num(); ++i)
	{
		if (ThrusterParticleEffects[i])
		{
			const float ThrustScale = FMath::Lerp(
				...
				ShipMovementComponent->GetShipCurrentSpeed() / ShipMovementComponent->GetMaxSpeed());
...
void AShipPawn::OnShipCollision(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (IsValid(OtherActor) && OtherActor != this)
	{
		...
		float Damage = FMath::GetMappedRangeValueClamped(FVector2D(ShipMovementComponent->GetShipMinSpeed(), ShipMovementComponent->GetMaxSpeed()), FVector2D(MinCollisionDamage, MaxCollisionDamage), Speed);
